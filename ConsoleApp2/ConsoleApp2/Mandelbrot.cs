...
namespace MandelbrotZoomer.Businesslayer
{
    public class Mandelbrot
    {
        ...
        static void MandelbrotKernel(
            Index1 index,
            int width, int height, float offsetx, float offsety, float zoom, int max_iterations,
            ArrayView<int> output)
        {
            ...
            float v_b = 1.0f;
            */
            float h_a = (-2f / zoom) + offsetx;
            float h_b = (1f / zoom) + offsetx;
            float v_a = (-1f / zoom) + offsety;
            float v_b = (1f / zoom) + offsety;
        ...
        private static System.Action<Index1, int, int, float, float, float, int, ArrayView<int>> mandelbrot_kernel;
        ...
        public static void CompileKernel(bool withCUDA)
        {
            ...
            mandelbrot_kernel = accelerator.LoadAutoGroupedStreamKernel<
                Index1, int, int,float,float,float ,int, ArrayView<int>>(MandelbrotKernel);
        }
        ...
        /// <param name="height"></param>
        /// <param name="max_iterations"></param>
        public static void CalcGPU(int[] buffer, int width, int height, float offsetx, float offsety, float zoom, int max_iterations)
        {
            int num_values = buffer.Length;
            var dev_out = accelerator.Allocate<int>(num_values);

            // Launch kernel
            mandelbrot_kernel(num_values, width, height, offsetx,  offsety,  zoom, max_iterations, dev_out.View);
        ...
        /// <param name="height"></param>
        /// <param name="max_iterations"></param>
        public static void CalcCPU(int[] buffer, int width, int height, float offsetx, float offsety, float zoom, int max_iterations)
        {
            /*
            float h_a = -1.40f;
            float h_b = -1.35f;
